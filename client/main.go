...
import (
	...
	"learn-go-grpc/common/config"
	"learn-go-grpc/common/model"
	"log"
)
...
func serviceUser() model.UsersClient {
	...
	conn, err := grpc.Dial(port, grpc.WithInsecure())
	if err != nil {
		log.Fatal("could not connet to ", port, err)
	}
...
func main()  {
	...
	// register user
	_, _ = user.Register(context.Background(), &user1)
	_, _ = user.Register(context.Background(), &user2)
	...
	_, _ = garage.Add(context.Background(), &model.GarageAndUserId{
		UserId:               user1.Id,
		Garage:               &garage1,
	})
